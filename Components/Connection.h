...
class Connection :	public Component
{
	...
	GraphicsInfo load_information;
public:
	//Connection(const GraphicsInfo &r_GfxInfo, Component *pS=NULL,Component *pD=NULL, int Pin=0);
	Connection(const GraphicsInfo &r_GfxInfo, OutputPin *pSrcPin,InputPin *pDstPin, Component*pDstCmpnt,GraphicsInfo li);
	Component *GetDstComp();	// Returns the destination component of a connection
	virtual void Operate() ;	//Calculates the output according to the inputs
	virtual void Draw(Output* pOut);	//for each component to Draw itself
	virtual void Draw(GraphicsInfo, bool ,Output* pOut);	//for each component to Draw itself
	virtual int no();
